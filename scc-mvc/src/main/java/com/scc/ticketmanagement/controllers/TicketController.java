        ...
        if (ticket != null) {
            ...
            switch (ticket.getStatusid()) {
                case Constant.STATUS_ASSIGN:
                    detail.setStatusid("Assigned");
                    break;
                case Constant.STATUS_INPROCESS:
                    detail.setStatusid("Inprocess");
                    break;
                case Constant.STATUS_CLOSE:
                    detail.setStatusid("Close");
                    break;
                case Constant.STATUS_SOLVED:
                    detail.setStatusid("Solved");
                    break;
                case Constant.STATUS_EXPIRED:
                    detail.setStatusid("Expired");
                    break;
            }
...
//        ticket.setDuetime(new Timestamp(new Date().getTime()));
    ...
    @RequestMapping("ticket/overviewClassify")
    public List<Long> countAndClassifyTicket(HttpServletRequest request) {

        try {
            HttpSession session = request.getSession();
            String username = (String) session.getAttribute("username");
            int brandId = userRepository.findUserByUsername(username).getBrandid();
            List<Long> rs = new ArrayList<>();
            rs.add(ticketRepository.countTicketByBrandIdandStatus(brandId, Constant.STATUS_ASSIGN));
            rs.add(ticketRepository.countTicketByBrandIdandStatus(brandId, Constant.STATUS_INPROCESS));
            rs.add(ticketRepository.countTicketByBrandIdandStatus(brandId, Constant.STATUS_SOLVED));
            rs.add(ticketRepository.countTicketByBrandIdandStatus(brandId, Constant.STATUS_CLOSE));
            rs.add(ticketRepository.countTicketByBrandIdandStatus(brandId, Constant.STATUS_EXPIRED));
